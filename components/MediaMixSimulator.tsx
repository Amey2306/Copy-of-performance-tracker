...
import { PieChart, TrendingUp, AlertCircle, CheckCircle2, Plus, Trash2, ChevronDown, RotateCcw, ChevronUp, X } from 'lucide-react';
...
interface Props {
  ...
  viewMode: ViewMode;
  manualBudget?: number;
  taxPercent: number; // Added taxPercent
...
export const MediaMixSimulator: React.FC<Props> = ({ 
  channels, metrics, viewMode, manualBudget, taxPercent, onUpdateChannel, onUpdateManualBudget, onAddChannel, onDeleteChannel 
}) => {
  ...
  const forecastedData = channels.map(ch => {
    ...
    // Effective CPL for calculation (Must align with Budget basis - Gross vs Net)
    const storedCpl = ch.estimatedCpl || 0;
    const effectiveCpl = storedCpl * taxMult;

    const leads = effectiveCpl > 0 ? budget / effectiveCpl : 0;
    
    const capi = leads * ((ch.capiPercent || 0) / 100);
    const ap = capi * ((ch.capiToApPercent || 0) / 100);
    const ad = ap * ((ch.apToAdPercent || 0) / 100);

    return { ...ch, budget, leads, capi, ap, ad, effectiveCpl };
  });
  ...
  const PRESET_GROUPS = [
    { label: "Digital", items: ["LinkedIn", "YouTube", "Google Discovery", "OTT Platforms", "Native Ads"] },
    { label: "Offline / ATL", items: ["Print / Newspaper", "Hoardings / OOH", "Radio", "Cinema Ads"] },
    { label: "BTL / Direct", items: ["Channel Partners", "Referral", "SMS / WhatsApp"] }
  ];
        ...
        <div className="flex-1 overflow-x-auto custom-scrollbar">
          <table className="w-full text-xs text-left border-separate border-spacing-0">
            <thead className="bg-slate-950 text-slate-500 border-b border-slate-800 uppercase font-bold whitespace-nowrap">
              <tr>
                <th className="px-4 py-3 sticky left-0 bg-slate-950 z-30 border-r border-slate-800 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.5)]">Channel</th>
                <th className="px-2 py-3 text-center w-20 bg-slate-900/50 border-b border-slate-800">Alloc %</th>
                <th className="px-3 py-3 text-right min-w-[120px] bg-slate-900/50 text-emerald-400 border-b border-slate-800">Budget (Edit)</th>
                <th className="px-2 py-3 text-right w-24 border-b border-slate-800">Est. CPL</th>
                <th className="px-3 py-3 text-right text-indigo-300 border-b border-slate-800">Proj. Leads</th>
                
                <th className="px-2 py-3 text-center w-20 bg-slate-900/30 border-l border-b border-slate-800">CAPI %</th>
                <th className="px-2 py-3 text-right text-indigo-200 bg-slate-900/30 border-b border-slate-800">Proj. CAPI</th>
                
                <th className="px-2 py-3 text-center w-20 bg-slate-900/30 border-l border-b border-slate-800">CAPI-AP %</th>
                <th className="px-2 py-3 text-right text-purple-200 bg-slate-900/30 border-b border-slate-800">Proj. AP</th>

                <th className="px-2 py-3 text-center w-20 bg-slate-900/30 border-l border-b border-slate-800">AP-AD %</th>
                <th className="px-2 py-3 text-right text-pink-200 bg-slate-900/30 border-b border-slate-800">Proj. AD</th>

                <th className="px-2 py-3 text-center bg-slate-950 sticky right-0 z-50 shadow-[-2px_0_5px_-2px_rgba(0,0,0,0.5)] border-l border-b border-slate-800 text-[10px] text-slate-600 w-14">Action</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800">
              {forecastedData.map(channel => (
                <tr key={channel.id} className="hover:bg-slate-800/50 transition-colors group">
                  <td className="px-4 py-2 font-medium text-slate-200 sticky left-0 bg-slate-900 group-hover:bg-slate-800 z-20 border-r border-slate-800 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.5)]">
                     <input 
                      type="text"
                      value={channel.name}
                    ...
                    <div className="flex flex-col items-center gap-1">
                        ...
                        <div className="w-full h-1 bg-slate-800 rounded-full overflow-hidden">
                            <div 
                                ...
                                style={{ width: `${Math.min(channel.allocationPercent || 0, 100)}%` }}
                            ></div>
                        </div>
                    </div>
                  </td>
                  ...
                  <td className="px-2 py-2">
                    <input
                      type="number"
                      value={channel.effectiveCpl === 0 ? '' : Math.round(channel.effectiveCpl)}
                      placeholder="0"
                      onChange={(e) => onUpdateChannel(channel.id, 'estimatedCpl', (parseFloat(e.target.value) || 0) / taxMult)}
                    ...
                    {deleteConfirmId === channel.id ? (
                      <div className="flex items-center justify-center gap-1 animate-in fade-in zoom-in duration-200">
                         <button 
                           ...
                           className="px-2 py-1 bg-red-600 hover:bg-red-500 text-white text-[10px] font-bold rounded shadow-lg shadow-red-900/50 transition-colors"
                         >
                           Confirm
                         </button>
                         <button 
            ...
            <tfoot className="bg-slate-950 border-t border-slate-800 font-bold sticky bottom-0 z-20 shadow-[0_-2px_5px_-2px_rgba(0,0,0,0.5)]">
              <tr>
                <td className="px-4 py-3 text-white sticky left-0 bg-slate-950 z-30 border-r border-slate-800 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.5)]">Total</td>
                ...
                <td className="px-2 py-3 sticky right-0 bg-slate-950 z-50 border-l border-slate-800 shadow-[-2px_0_5px_-2px_rgba(0,0,0,0.5)]"></td>
              </tr>
            </tfoot>
          </table>
        </div>
        ...
        <div className="bg-slate-950/50 p-3 border-t border-slate-800 flex justify-between items-center">
           <div className="relative">
             <button 
               ...
               className={`flex items-center gap-2 text-xs font-bold px-3 py-1.5 rounded transition-all duration-200 ${showAddMenu ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/50' : 'text-indigo-400 hover:text-indigo-300 bg-indigo-900/20 hover:bg-indigo-900/40'}`}
             >
               <Plus className="w-3.5 h-3.5" /> Add Channel {showAddMenu ? <ChevronDown className="w-3 h-3" /> : <ChevronUp className="w-3 h-3" />}
             </button>
             {showAddMenu && (
               <div 
                 onClick={(e) => e.stopPropagation()} 
                 className="absolute bottom-full left-0 mb-2 w-56 bg-slate-800 border border-slate-700 shadow-2xl rounded-lg overflow-hidden z-30 max-h-80 overflow-y-auto custom-scrollbar animate-in fade-in zoom-in-95 duration-200"
               >
                  {PRESET_GROUPS.map((group, idx) => (
                    <div key={group.label}>
                        {idx > 0 && <div className="border-t border-slate-700/50 my-1"></div>}
                        <div className="text-[10px] uppercase font-bold text-slate-500 px-3 py-1.5 bg-slate-900/50 tracking-wider">{group.label}</div>
                        {group.items.map(name => (
                            <button 
                              key={name}
                              onClick={() => { onAddChannel(name); setShowAddMenu(false); }}
                              className="w-full text-left px-3 py-2 text-xs text-slate-300 hover:bg-indigo-600 hover:text-white transition-colors flex items-center justify-between group/item"
                            >
                              {name} <Plus className="w-3 h-3 opacity-0 group-hover/item:opacity-100 transition-opacity" />
                            </button>
                        ))}
                    </div>
                  ))}
                  ...
                  <button 
                    ...
                    className="w-full text-left px-3 py-2.5 text-xs text-indigo-400 font-bold hover:bg-slate-700 transition-colors flex items-center gap-2"
                  >
                    <Plus className="w-3.5 h-3.5" /> Custom Channel...
                  </button>
               </div>
             )}
           </div>
          ...
          <div className="space-y-6">
            ...
            <div className="pt-4 border-t border-slate-800">
               ...
               <div className="flex items-baseline gap-2">
                 ...
                 <span className="text-xs text-slate-500">vs Plan: {formatCurrency(metrics.cpw * taxMult)}</span>
               </div>
