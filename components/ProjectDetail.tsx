...
import { Project, TabView, CalculatedMetrics, PlanningData, WeeklyData, WeeklyActuals, ViewMode, MediaChannel, ChannelPerformance, User, UserRole } from '../types';
...
export const ProjectDetail: React.FC<Props> = ({ 
  project, metrics, viewMode, currentUser, onBack, onUpdatePlan, onUpdateWeek, onUpdateActual, 
  ...
  // Managers cannot edit Plan, WoW, or Media Mix. SM can edit Media/WoW but not Plan.
    ...
    <div className="space-y-6">
      ...
      <div className="transition-all duration-300">
        ...
        <div className="flex justify-end mb-2 gap-2">
            ...
            {/* Lock Button - Only GM can toggle */}
            {(activeTab === TabView.PLANNING || activeTab === TabView.WOW_PLAN) && (
              <button 
                onClick={() => currentUser.role === UserRole.GM && onToggleLock(project.id)}
                disabled={currentUser.role !== UserRole.GM}
                className={`flex items-center gap-2 text-xs font-bold px-3 py-1.5 rounded border shadow-sm transition-colors ${project.isLocked ? 'bg-amber-950/40 border-amber-900 text-amber-400' : 'bg-slate-900 border-slate-700 text-slate-300'} ${currentUser.role !== UserRole.GM ? 'cursor-not-allowed opacity-70' : 'hover:bg-slate-800'}`}
              >
        ...
        {activeTab === TabView.PLANNING && (
          <div className="relative">
             {isPlanReadOnly && !project.isLocked && <div className="absolute top-2 right-2 z-10 text-xs font-bold text-slate-500 bg-slate-900/80 px-2 py-1 rounded border border-slate-700">Read Only View</div>}
             <PlanningSection 
                data={project.plan} 
                metrics={metrics} 
        ...
        {activeTab === TabView.MEDIA_MIX && (
          <div className={isMediaReadOnly ? "pointer-events-none opacity-90" : ""}>
             <MediaMixSimulator 
