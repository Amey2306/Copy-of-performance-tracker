...
import { TrendingUp, Users, Footprints, IndianRupee, ArrowUpRight, ArrowDownRight, Target, Activity, Info, Filter, PieChart, BarChart3, AlertCircle, Download, Layers, Briefcase } from 'lucide-react';
...
export const VisualDashboard: React.FC<Props> = ({ projects, viewMode }) => {
  ...
  // Deficit Calculation
  const TARGET_DIGITAL_CONTRIB_PERCENT = 10; // 10% Fixed Benchmark
  const targetDigitalRevenue = revTotal * (TARGET_DIGITAL_CONTRIB_PERCENT / 100);
  const digitalDeficit = targetDigitalRevenue - revDigital;
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-12">
      ...
      {/* --- NEW SECTION: BUSINESS VERTICAL & DEFICIT ANALYSIS --- */}
      <div className="bg-slate-900 rounded-2xl border border-slate-800 p-6 shadow-xl">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 gap-4">
           <div>
             <h3 className="text-lg font-bold text-white flex items-center gap-2">
               <Layers className="w-5 h-5 text-indigo-400" /> 
               Business Vertical Overview & Deficit Tracker
             </h3>
             <p className="text-xs text-slate-500 mt-1">
               Analysis of Revenue contribution by Channel Partner vs Direct Verticals (Digital, Referral, Brand, Presales).
             </p>
           </div>
           <div className="text-right">
             <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Total Business</div>
             <div className="text-2xl font-black text-white">{formatRev(revTotal)}</div>
           </div>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
           {/* 1. Deficit Card */}
           <div className={`p-6 rounded-xl border ${digitalDeficit > 0 ? 'bg-red-950/20 border-red-900/50' : 'bg-emerald-950/20 border-emerald-900/50'} relative overflow-hidden flex flex-col justify-center`}>
              <div className="relative z-10">
                 <div className="flex justify-between items-center mb-2">
                    <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Digital Contribution</span>
                    <span className="text-xs font-bold text-slate-500">Target: {TARGET_DIGITAL_CONTRIB_PERCENT}%</span>
                 </div>
                 
                 <div className="flex items-baseline gap-2 mb-1">
                   <span className={`text-4xl font-black ${digitalDeficit > 0 ? 'text-red-400' : 'text-emerald-400'}`}>
                     {currentDigitalContribPercent.toFixed(1)}%
                   </span>
                 ...
                 <div className="w-full bg-slate-800 rounded-full h-2 mb-4 overflow-hidden">
                    <div 
                      className={`h-full rounded-full ${digitalDeficit > 0 ? 'bg-red-500' : 'bg-emerald-500'}`} 
                      style={{ width: `${Math.min((currentDigitalContribPercent / TARGET_DIGITAL_CONTRIB_PERCENT) * 100, 100)}%` }}
                    ></div>
                 </div>

                 {digitalDeficit > 0 ? (
                    <div className="flex items-center gap-2 text-red-300 text-sm font-bold bg-red-950/40 p-2 rounded-lg border border-red-900/30">
                       <AlertCircle className="w-4 h-4" /> 
                       Deficit: {formatRev(digitalDeficit)}
                    </div>
                 ) : (
                    <div className="flex items-center gap-2 text-emerald-300 text-sm font-bold bg-emerald-950/40 p-2 rounded-lg border border-emerald-900/30">
                       <TrendingUp className="w-4 h-4" /> 
                       Surplus: {formatRev(Math.abs(digitalDeficit))}
                    </div>
                 )}
              </div>
           </div>

           {/* 2. Vertical Breakdown Stacked Bar */}
           <div className="lg:col-span-2 flex flex-col justify-center">
              <div className="mb-4 flex justify-between items-center">
                <span className="text-xs font-bold text-slate-400 uppercase">Revenue Split</span>
              </div>
              
              {/* Main Split: Direct vs CP */}
              <div className="flex h-12 rounded-lg overflow-hidden w-full font-bold text-xs text-white shadow-lg mb-2">
                 {/* Direct Section */}
                 <div 
                   className="bg-indigo-600 flex items-center justify-center relative group cursor-help transition-all hover:bg-indigo-500"
                   style={{ width: `${revTotal > 0 ? (revDirect / revTotal) * 100 : 0}%` }}
                 >
                    <span className="truncate px-2">Direct {formatRev(revDirect)}</span>
                 </div>
                 {/* CP Section */}
                 <div 
                   className="bg-purple-600 flex items-center justify-center relative group cursor-help transition-all hover:bg-purple-500"
                   style={{ width: `${revTotal > 0 ? (revCP / revTotal) * 100 : 0}%` }}
                 >
                    <span className="truncate px-2">Channel Partner {formatRev(revCP)}</span>
                 </div>
              </div>

              {/* Direct Sub-Verticals Legend/Breakdown */}
              <div className="bg-slate-950/50 rounded-lg p-3 border border-slate-800 flex flex-wrap gap-4 justify-between items-center text-xs">
                  <div className="flex items-center gap-2">
                     <div className="w-2 h-2 rounded-full bg-indigo-500"></div>
                     <span className="text-slate-400 font-medium">Direct Total:</span>
                     <span className="text-white font-bold">{((revDirect/revTotal)*100 || 0).toFixed(1)}%</span>
                  </div>
                  ...
                  <div className="flex gap-4 flex-wrap">
                      <div className="flex flex-col">
                        <span className="text-[10px] text-slate-500 uppercase font-bold">Digital</span>
                        <span className="text-white font-bold">{formatRev(revDigital)}</span>
                      </div>
                      <div className="flex flex-col">
                        <span className="text-[10px] text-slate-500 uppercase font-bold">Referral</span>
                        <span className="text-white font-bold">{formatRev(revReferral)}</span>
                      </div>
                      <div className="flex flex-col">
                        <span className="text-[10px] text-slate-500 uppercase font-bold">Brand</span>
                        <span className="text-white font-bold">{formatRev(revBrand)}</span>
                      </div>
                      <div className="flex flex-col">
                        <span className="text-[10px] text-slate-500 uppercase font-bold">Presales</span>
                        <span className="text-white font-bold">{formatRev(revPresales)}</span>
                      </div>
                  </div>
              </div>
           </div>
