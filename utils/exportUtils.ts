...
export const exportMasterReport = (projects: Project[], viewMode: ViewMode) => {
  const data = projects.map(p => {
    ...
    const directUnits = achDigBookings + achPresalesBookings + achBrandBookings + achReferralBookings;
    const cpUnits = achCPBookings;
    const totalUnits = directUnits + cpUnits;
    ...
    return {
      ...
      "Brand Bookings": achBrandBookings,
      "Referral Bookings": achReferralBookings,
      "CP Bookings": achCPBookings,
      "Total Units": totalUnits,
      "Direct BV (Cr)": (directUnits * p.plan.ats),
      "CP BV (Cr)": (cpUnits * p.plan.ats),
      "Total BV (Cr)": (totalUnits * p.plan.ats)
    };
  });
